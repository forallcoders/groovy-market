...
export function quoteMarketBuy({
  ...
  for (const ask of asks) {
    ...
    if (remainingUSDC >= costForAllShares) {
      ...
      usedAsks.push({ price: ask.price, quantity: availableShares * (10 ** outcomeTokenDecimals) });
    } else {
      // Can only buy a portion of shares at this price level
      const partialShares = remainingUSDC / ask.price;
      totalShares += partialShares;
      usedAsks.push({ price: ask.price, quantity: partialShares * (10 ** outcomeTokenDecimals) });
      remainingUSDC = 0;
      break;
    }
  ...
  const takerAmount = BigInt(Math.floor(sharesToReceive * (10 ** outcomeTokenDecimals)));
...
export function quoteMarketSell({
  shares,
  bids, // Note: these should be the bids (buy orders), not asks
  outcomeTokenDecimals = 6,
  usdcDecimals = 6,
  ...
  for (const bid of bids) {
    ...
    if (remainingShares >= bidQuantity) {
      ...
      usedBids.push({ price: bid.price, quantity: bidQuantity * (10 ** outcomeTokenDecimals) });
    } else {
      // Can only sell a portion to this bid
      totalProceeds += remainingShares * bid.price;
      usedBids.push({ price: bid.price, quantity: remainingShares * (10 ** outcomeTokenDecimals) });
      remainingShares = 0;
      break;
    }
  ...
  const takerAmount = BigInt(Math.floor(proceedsToReceive * (10 ** usdcDecimals)));
  
  const averagePrice = shares > 0 ? totalProceeds / (shares - remainingShares) : 0;
